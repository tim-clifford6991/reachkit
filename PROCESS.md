...
2. Read the issue, then **only** what it cites: the `BUILD.md` § it names, the `ARCHITECTURE.md` rows for the paths you touch, and — for a screen — UI-SPEC.md §1 and that screen's `S<id>`. **`DECISIONS.md` is grepped, never read whole**: read `/root/ops/reachkit/state/decisions-index.md` (generated), then grep `DECISIONS.md` by topic — `grep -in '<the nouns in your issue>' DECISIONS.md`. Reading 250 rulings to build one issue is how an implementer spends a session on a document instead of on the product.
