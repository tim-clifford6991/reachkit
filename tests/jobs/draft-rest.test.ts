...
const { regenerateDraft } = await import("../../src/app/(account)/app/calendar/publishing-actions");
...
describe("#788 — a draft the rules stop twice ends in needs_attention, on one row", () => {
  it("the evening tick leaves one row for the date, resting with its rules, and the opportunity open again", async () => {
