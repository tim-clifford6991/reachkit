...
const facts = (over: Partial<OverviewFacts> = {}): OverviewFacts => ({
  ...
  supply: { exhausted: false, unmeasured: false, measuring: false, outsized: false, short: false, firstArrivalShortfall: false },
  waiting: [],
...
describe("the assembly is pure, and reads nothing", () => {
  ...
  it("it states one supply statement or none, and at most two alerts", () => {
    const model = assembleOverview(
      facts({ supply: { exhausted: true, unmeasured: false, measuring: false, outsized: false, short: true, firstArrivalShortfall: true } })
    );
