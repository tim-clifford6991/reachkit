...
describe("REQ-093 c5 — the registry renders with every model unavailable", () => {
  ...
  it("every non-owner-owed key returns its literal through copy(), with zero import path to a language model", () => {
    ...
    //
    // 2026-09-08, issue #373: S9's two answered arms and its glass card,
    ...
    //
    // 2026-09-09, the master's review of #369: the four spec rows the set
    // draws on S4, unbracketed, are approved copy for that screen too — a
    // second wording of the same four terms, split from the report card's
    // ruled lines rather than overwriting them. 346 + 4 = 350.
    //
    // 2026-09-09, issue #374 and the master's review of #391 — the approved
    // S18, under ruling 11a. Twelve keys arrive written: the "How your pages
    // sound" card's five (`settings.voice.title`, `.never-claim`,
    // `.add-claim`, `.add`, `.filter-note`), REQ-075's
    // `settings.notifications.always-on`, REQ-073 c2's
    ...
    //
    // 2026-09-09, issue #375: S19's four — the byline, the source line, the
    // canonical note and the customer's footer line. All four are new and
    // all four are written: the set spells every one of them unbracketed,
    // and what it brackets on that screen (the brand, the category, the
    // imprint line) is the customer's data and not the owner's copy.
    // 364 + 4 = 368.
    expect(ruled.length).toBe(368);
...
describe("owner-owed and empty agree both ways", () => {
  ...
  it("counts: 117 owner-owed, 244 awaiting copy, 368 ruled, 729 total (rule 5.5 — the index states its own coverage)", () => {
    ...
    // 681 total.
    // And S9's arms, issue #373: six keys — four owed (the sent and expired
    // heads, the resend word and the new-link control, all bracketed in the
    ...
    // 699 + 4 = 703 total.
    // And the master's review of the same issue: the four spec rows in S4's
    // own words, all four written, none owed — the report card's ruled four
    // stay exactly where they are. 117 owner-owed, 240 awaiting copy,
    // 346 + 4 = 350 ruled, 703 + 4 = 707 total.
    expect(OWNER_OWED.length).toBe(117);
    //
    // #374 adds thirteen keys of its own: twelve written under ruling 11a
    // (above) and one owed — `settings.voice.placeholder`, which the
    // approved set brackets, so the words are the owner's. It takes the
    // marker rather than the empty value: the field renders on a screen the
    // customer reaches, and a placeholder that throws would take the card
    ...
    // 720 + 5 = 725 total.
    //
    // And S19's own four, issue #375: the hosted page's byline, source
    // line, canonical note and footer line — the first sentences that
    // surface has ever carried, every one of them written (11a).
    // 117 owner-owed, 244 awaiting copy, 364 + 4 = 368 ruled,
    // 725 + 4 = 729 total.
    expect(AWAITING_COPY.length).toBe(244);
    expect(Object.keys(COPY).length - OWNER_OWED.length - AWAITING_COPY.length).toBe(368);
    expect(Object.keys(COPY).length).toBe(729);
