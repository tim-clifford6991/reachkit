...
describe(
  'BP-010 `## Error & edge behavior` — "Determinism is a test, not an aspiration: the same HTML measured twice produces byte-identical output (BUILD §16 milestone 2). No JS execution, no clock in the parse, no locale-dependent collation."',
  () => {
    const fixture = `
      <html><head>
...
describe(
  'BP-010 `## Error & edge behavior` — "Empty denominators read 0, never null (BUILD §5): a page with no headings scores `zero`, not `unmeasured`."',
  () => {
...
describe(
  'BUILD §5 — "question-shaped = ends `?` or opens with how/what/why/when/where/which/who/can/do/does/is/are"',
  () => {
...
describe(
  'BUILD §5 — "`directAnswers` = question headings whose first block is 40–320 visible chars ÷ all headings × 100"',
  () => {
...
describe(
  'BUILD §5 — "`evidenceDensity` = saturating log curve over (numerals + dates + outbound citations per 1k chars)" — the counts only; the curve is WO-252\'s',
  () => {
