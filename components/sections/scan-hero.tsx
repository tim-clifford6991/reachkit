 ...
 * meta line, and proof card. The proof card is a browser-framed slice of the
 * real paid dashboard — gauge + band chip, you-vs-rival bars, the next best
 * action, and the search-gap rows rivals win. Static demo data (bloom.io),
 * lightweight inline SVG/HTML only (no intel-kit import — bundle budget).
 */
    ...
    <section style={{ position: "relative", overflow: "hidden", background: "radial-gradient(1100px 480px at 50% -8%, var(--c-soft) 0%, rgba(242,238,255,0) 62%), var(--c-bg)" }}>
      <div style={{ maxWidth: 1180, margin: "0 auto", padding: "70px 28px 56px", textAlign: "center" }}>
        ...
        <h1 style={{ fontFamily: SG, fontWeight: 700, fontSize: "clamp(2.4rem, 5.4vw, 57px)", lineHeight: 1.04, letterSpacing: "-0.035em", margin: "22px auto 0", maxWidth: 860, textWrap: "balance" } as React.CSSProperties}>
          Your competitors are being found. You aren&apos;t. <span style={{ color: "var(--c-action)" }}>See exactly why.</span>
        </h1>

        <p style={{ fontSize: 19, lineHeight: 1.55, color: "var(--c-muted)", maxWidth: 620, margin: "20px auto 0", textWrap: "pretty" } as React.CSSProperties}>
        ...
        <div style={{ maxWidth: 800, margin: "44px auto 0", background: "var(--c-surface)", border: "1px solid var(--c-line)", borderRadius: 16, overflow: "hidden", boxShadow: "0 30px 80px -28px rgba(40,33,84,0.22)", textAlign: "left" }}>
