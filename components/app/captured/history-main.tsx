...
function Chart({ history, markers, current }: { history: HistoryPoint[]; markers: Marker[]; current: number }) {
  if (history.length < 2) {
    return <div style={{ height: 180, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, color: "var(--c-faint)" }}>Your score history builds with each weekly scan.</div>;
  }
...
export function HistoryMain(p: HistoryMainProps) {
  ...
  return (
    <div>
      <div style={{ background: "var(--c-surface)", border: "1px solid var(--c-line)", borderRadius: 16, padding: "22px 24px", marginBottom: 20 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <div style={{ fontWeight: 700, fontSize: 16, fontFamily: SG }}>Score over time</div>
          <span style={{ fontSize: 11.5, fontWeight: 600, color: "var(--c-action)", background: "var(--c-soft)", padding: "4px 10px", borderRadius: 7 }}>{p.history.length} weekly scan{p.history.length === 1 ? "" : "s"}</span>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.3fr 1fr", gap: 20 }}>
        ...
        <div style={{ background: "var(--c-surface)", border: "1px solid var(--c-line)", borderRadius: 16, padding: "20px 22px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            {p.timeline.map((r, i) => {
              ...
              const dotColor = i === 0 ? "var(--c-action)" : last ? "#D9D6E4" : "#C7BCF5";
              const dColor = r.delta === null ? "var(--c-faint)" : r.delta >= 0 ? "#1F9D5B" : "#E5484D";
              return (
                <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 14, paddingBottom: 16 }}>
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flex: "0 0 auto" }}>
                    ...
                    {!last && <span style={{ width: 2, flex: "1 1 0%", background: "var(--c-line2)", minHeight: 22 }} />}
                  </div>
                  <div style={{ flex: "1 1 0%", paddingBottom: 4 }}>
                    ...
                    <div style={{ fontSize: 12.5, color: "var(--c-faint)", marginTop: 2 }}>{r.note}</div>
                  </div>
                </div>
              );
            })}
        ...
        <div style={{ background: "var(--c-surface)", border: "1px solid var(--c-line)", borderRadius: 16, padding: "20px 22px" }}>
          ...
          <div style={{ fontSize: 12.5, color: "var(--c-faint)", marginBottom: 14 }}>How accurate our impact estimates have been.</div>
          {p.accuracy.length === 0 ? (
            <div style={{ fontSize: 13, color: "var(--c-faint)", padding: "12px 0" }}>Accuracy builds as you verify shipped fixes.</div>
          ) : (
            <>
                ...
                {p.accuracy.map((a, i) => (
                  <div key={i}>
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, marginBottom: 5 }}>
                      ...
                      <span style={{ fontFamily: JM, color: "var(--c-muted)" }}>+{a.pred} → +{a.actual}</span>
                    </div>
                    <div style={{ height: 6, borderRadius: 3, background: "var(--c-fill)", overflow: "hidden" }}><div style={{ height: "100%", width: `${Math.round((a.actual / maxPred) * 100)}%`, background: "#1F9D5B", borderRadius: 3 }} /></div>
                  </div>
                ))}
              </div>
              {p.modelAccuracy !== null && (
                <div style={{ marginTop: 16, paddingTop: 14, borderTop: "1px solid var(--c-line2)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontSize: 13, color: "var(--c-muted)", fontWeight: 500 }}>Model accuracy</span>
