...
4. Implement the issue and its tests. `// BUILD §x.y` at the top of each new module. Copy: the approved set's unbracketed strings are approved (ruling 11a); anything else is a key with `TODO(copy)`, listed in the PR. Never invent a sentence. Never edit an owner file (`BUILD.md`, `DECISIONS.md`, `ARCHITECTURE.md`, `.github/**`, `scripts/**`, `eslint.config.mjs`, `vitest.config.ts`, `src/lib/config/`); a documented fact you change is named under *Corpus* in the PR body.
