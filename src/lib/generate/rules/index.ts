...
//
// DECISIONS 2026-09-06: Generation: hard rules are code, not prompts; `drafts.attribution`
//   stays null until a customer states a business name at setup (no column records one today)
//   — the profile's measured brand token is never used as an attribution, because that would
//   be the generated byline the rule forbids; the grounded passage is frozen by trigger after
//   write (url and read date stay correctable); no outstanding/recheck flag column — the sweep
//   is served off a partial index so nothing can go stale. — #135
