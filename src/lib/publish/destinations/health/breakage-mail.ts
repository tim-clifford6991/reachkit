...
/**
 * The moment this customer was last known to have signed in, or `null`
 * where they never have.
 *
 * **It reads `users.first_signed_in_at`, which is a lower bound.** §13's
 * identity half — the sign-in links and the session they redeem into — is
 * issue #35, and nothing in this build records a *second* sign-in, so
 * today the first is the last and the bound is exact. When #35 lands, this
 * one read moves to the moment it records and nothing else here changes.
 *
 * The direction of the error, while it is a bound, is the safe one: a
 * customer who has signed in *since* the breakage always has a
 * `first_signed_in_at` at or before it, so the only mistake it can make is
 * writing to somebody who came back — never staying silent about a
 * destination nobody has seen.
 */
async function ownerOf(siteId: string): Promise<{ email: string; lastSignInAt: Date | null } | null> {
  ...
  const stamp = owner.data.first_signed_in_at;
  return {
    email: owner.data.email,
    lastSignInAt: stamp === null ? null : new Date(stamp),
  };
}
