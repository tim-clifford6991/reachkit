...
export async function sendEmail(m: SendInput): Promise<SendResult> {
  ...
  // §8: a sentence the owner has not written sends nothing at all. `copy()`
  // above already refuses an empty value; the other owner-owed standing
  // renders its marker, and this is what stops that reaching a reader.
