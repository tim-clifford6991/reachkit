...
|------|--------|---------|----------------|------------|
| **Discoverability Score (headline, v6)** | HTML fetch (on-page) **×** ONE `ranked_keywords` call (search presence + raw `keywordsRanked` footprint) — all free-computable | `scans.score_total` (`score_version 6`) | `discoverabilityScore(onPageReadiness, searchPresence, keywordsRanked)` = **geomean** of (a) `headlineScore` = `registryScore` over the FIXED 8 on-site signals (`FIXED_BASIS_SIGNAL_KEYS`) and (b) `searchVisibility.score` **floored by `findabilityFloor(keywordsRanked)`** (v6: a broadly-ranking site can't read "Invisible" on a classifier miss — x.com 15M kw was 4/100). All inputs identical free↔paid → **the unified number never moves on upgrade**. Geometric so BOTH must be strong (a tidy page invisible in search reads low, e.g. 98 × 4 → 20). `searchPresence` floored at 1; `keywordsRanked` is a RAW count (not the frozen classifier). | Dashboard/report gauge + **two driver bars** ("On-page readiness" · "Search presence") |
...
| 2 | `findings` (`:99`) | `runExtract` (Haiku fact sheets) → `runSynth` **LITE** (**Haiku 4.5, render-only** — `SYNTH_MODEL_FREE`, `lite:true`): ONLY the **positioning mirror + `categorySeeds` + `intendedAudience`/`actualAudience`** the free report renders (~300 out tokens, ~3.4s vs ~17s full). The full findings/sampleAction the PAID plan is built from are regenerated by the deep pass (§5.2) → v1 score | 3–4 Haiku + 1 lite Haiku |
| 3 | `free-report` (`:147`) | `runFreeReport`: `headlineScore` over the 8 `FIXED_BASIS_SIGNAL_KEYS` (on-page driver) → **`gatherFreeSearchVisibility`** (`ranked_keywords` footprint + `search_volume` on the LLM `categorySeeds` → `report_payload.searchVisibility`) → **`discoverabilityScore(head, sv.score, sv.keywordsRanked)` persisted as `score_total` (`score_version 6`)** → `fallbackActionsFromSignals` → `buildFreeReport` | + ~2 DataForSEO calls (~$0.04) |
...
**Synth-model note (2026-07-19):** the findings step runs a **lite Haiku teaser**
(mirror + seeds only, ~3.4s) for EVERY tier — the free report renders nothing else
from synth. The **deep pass OWNS the full synthesis**: `runFullScan` re-runs
`runSynth` on **Sonnet** (`SYNTH_MODEL_FULL`) and `persistDeepSynth` overwrites the
teaser in `findings_payload` + the `findings` table, so `generateActions` (`:528`)
always builds the paid plan from full Sonnet findings — regardless of whether the
scan was paid-from-start or deepened from a free scan. (Deep synth model is a single
constant; Opus was considered and rejected — a bounded synthesis over fact sheets
where the A/B showed Haiku≈Sonnet, so more model power is low‑ROI vs upgrading the
Haiku action‑generation stage.)
...
`persistScanSignals`, `discoverabilityScore(headlineScore, searchVisibility.score, searchVisibility.keywordsRanked)` (v6, identical to free — 10a reuses persisted `searchVisibility`), `marketPositionScore`
...
- **Two-tier scan** — a fast, free lightweight report is produced first
  ...
  **unified Discoverability Score** (`score_version 6`) = `discoverabilityScore` =
