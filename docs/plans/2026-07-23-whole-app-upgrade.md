...
- **M3a — render-only contract reframe (DONE, zero cost).** Owner ruled M3 scope = **contract pillars, defer the LLM judge** (rival keywords aren't a paid pillar; the keyword spine is shared and already shipped). So:
  - **Competitors → the LESSONS surface.** Removed the metered per-rival `KeywordEdgeList` AND the keyword-based "their edge → your move" (`bestGapHit`/`moveLabel`) — BOTH surfaced the same raw unclassified rival keywords (cardpointers → `Counter: target "capital one" (9.1M/mo)`). The page is now referrers + channels: "Referrers to pursue" reframed as **"The lesson · pursue the referrers they have, you don't"** (add-to-plan outreach), edge is referrer/traffic framing only.
  - **Customers → ICP + communities.** Removed `DemandThemes` (keyword fork) and `PainBars` (review-derived — cut both tiers, O-7; the orphaned `pain-bars.tsx` + its DS mirror deleted). Added **"Communities to engage"** — each buyer community as a specific "Engage in «surface» → add to plan" move (contract pillar 3).
  - **Shared `add-to-plan.tsx`** extracted (one `useActionPlan`/`AddToPlanChip` for both lessons + communities; the dashboard's copy died in M1). DS mirrors reconciled (CompetitorsScreen/CustomersScreen/CompetitorEdgePanel updated, new AddToPlan mirror, coverage held at 28).
  - **Verified by RENDERING** the fixture pages (headless): Customers shows Communities-to-engage, no Demand-themes/pains; Competitors shows "The lesson"/"Their edge", no "Top keywords"/"capital one"/"Counter: target". 2012 tests green, tsc/arch/design clean.
- **M3b — cut the write-only producers.** ONE deep scan.
