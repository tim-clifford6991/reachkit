...
- 2026-09-18  A free scan gives up on a stuck vendor call sooner (owner ruling 2026-09-17, #875, on #873's diagnosis): a question's SERP request on the **free** path is abandoned after **5 s**, against the 10 s every other call keeps, and the calls it cuts short come back through #865's one retry after the sweep. `asking_the_twelve` is 13 s at a fan-out of four, so a call held to 10 s took a quarter of that stage for almost all of it and cost the questions behind it. An answer that arrives after its stage was abandoned is recorded while the report has not yet been composed — it was bought and paid for — and never after. No cap changes.
