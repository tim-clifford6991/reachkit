...
| Preview | **none since 2026-09-09**: the ignored-build step builds `main` only (`if [ "$VERCEL_GIT_COMMIT_REF" = "main" ]; then exit 1; else exit 0; fi`), because the Hobby plan's 100 builds/day were spent on PR previews; CI's renders comment (PROCESS §2.4) is the review surface. Deployment protection is off (Hobby, and Vercel's standard protection does not exempt a branch-bound custom domain) |
