...
| key | standing | where | the set says | fixed by | max |
|---|---|---|---|---|---|
| `mail.weekly.eyebrow`<br>`mail.ts` | marker | `EYEBROW` · `lib/mail/templates/weekly/index.ts` |  | SPEC.md §8 · Canvas: MailDigest | 67 — sibling `mail.weekly.body` |
| `mail.weekly.issues` `{count}`<br>`mail.ts` | marker | `ISSUES` · `lib/mail/templates/weekly/index.ts` |  | SPEC.md §9 · Canvas: MailDigest | 67 — sibling `mail.weekly.body` |
| `mail.weekly.issues.link`<br>`mail.ts` | marker | `ISSUES_LINK` · `lib/mail/templates/weekly/index.ts` |  | SPEC.md §9 · Canvas: MailDigest |  |
