...
**2 keys**, across 797 in the registry — **0 empty** (`copy()` throws on these: a mail with one does not send, a screen with one does not render) and **2 `TODO(copy)`** (these render the marker, in public, until they are written).
