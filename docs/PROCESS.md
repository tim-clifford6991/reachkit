...
4. Verify **locally what you touched, and let CI run the rest**: `npm run typecheck && npm run lint && npx vitest run --project node --project ui --maxWorkers=1`; the `db` project only when `supabase/` or `src/lib/db/` changed (substrate up for that run, down straight after — §5); the layout tests for the routes you changed, not the whole browser suite (`npx vitest run --project layout -t "<route>"`), regenerating only their baselines. The full layout and RLS suites are CI's job; a green local full run buys nothing CI does not repeat.
