...
|---|---|---|---|---|
| 1 | `DECISIONS.md` | **Product** rulings only (owner ruling 2026-09-11), dated, one line each, newest last; a superseded row is struck in place. A process ruling is a rule in `PROCESS.md` §8; an implementation ruling is a `// DECISIONS <date>:` comment at its module. The record before the split is `docs/archive/DECISIONS-full-2026-09-11.md`. Owner rulings are recorded verbatim in intent. Between batches the master's rulings wait in `docs/pending-decisions.md`. | a ruling is made (owner) or a design/engineering decision closes (master) — batched into a docs PR | the drift audit checks its format and order; `docs/pending-decisions.md` is emptied by each batch |
...
| 11a | `docs/archive/DECISIONS-full-2026-09-11.md` | `DECISIONS.md` exactly as it stood before the 2026-09-11 split — every row, product, process and implementation alike. Read-only; the place to look when a row cannot be found. | never | — |
| 12 | `docs/briefs/autopilot-quality-2026-09-10.md` | The owner's Autopilot quality & discoverability brief, copied verbatim: the source the DECISIONS rulings of 2026-09-10 (§1.1–§1.12) and the BUILD §4.6, §7, §8, §9 and §10 amendments of that date derive from. A brief is a source, not an authority: where BUILD and the brief disagree, BUILD wins once amended, and the difference is a DECISIONS line. | never (a later brief is a new dated file) | — (the rulings it produced are checked in `DECISIONS.md`) |
