...
| 2 | `SPEC.md` | **The specification.** What the product is: stack, design system, journey, screens, scoring, data, jobs, mail, payments, guardrails, env, build order, non-goals. | a DECISIONS row changes the spec (amended in the same or the next docs PR) | `scripts/drift-audit.mjs` (spec→code, routes, journeys, pins) |
| 3 | `SPEC.md` §4.8 | **The UI specification of record**, folded in on 2026-09-11 (formerly `docs/design/approved/full-set/UI-SPEC.md`): every screen `S<id>` and state, its modules, ranks, copy status and the requirement it satisfies, keeping its own § numbers so `UI-SPEC §n` resolves there. Its rendered form is `docs/design/approved/full-set/reachkit-full-screen-set.html`; its parent is the owner's artifact `reachkit-screen-system.html`; tokens in `tokens.css`; the directory's own index is `docs/design/approved/README.md`, the values it spends without a token name are `literals.md`, and the renders are `screens/*.png`. | the owner approves a new artifact | `tests/ui/design/*`, the layout suite's side-by-sides |
...
| 5 | `SPEC.md` §6.8 | The price book behind SPEC §6, folded in on 2026-09-11 (formerly `DATA-COSTS.md`), keeping its own § numbers so `DATA-COSTS §n` resolves there. | a vendor price is re-checked (dated) | `tests/pins.test.ts` |
...
| 9a | `docs/copy/owed.md` | **Generated** (`npm run copy:owed`): every copy key still owed — empty or `TODO(copy)` — by screen in the set's order, with where it sits, the set's bracketed hint, the requirement that fixes it and an implied length. The sheet the owner (or the master, if delegated) writes copy into; never typed into by hand. | a key is added, written or removed (regenerate in the same PR) | `tests/presentation/copy/owed-sheet.test.ts` (regenerate-and-diff, and the key list against the registry) |
| 9b | `LATER.md` | Deferred edge cases, v1.1 work and non-goals, with their issues. Nothing here is built; a line leaves only by an owner ruling. | the owner defers or admits something | the `later` label on its issues |
...
| 12 | `docs/archive/` | Dated, read-only sources whose rulings have been folded elsewhere — `autopilot-quality-2026-09-10.md`, the owner's Autopilot brief, whose §1 rulings are the DECISIONS rows of 2026-09-10 and whose §2–§10 are in SPEC §4.3, §4.6, §4.7, §7, §8 and §9. Its own index is `docs/archive/README.md`. A brief is a source, not an authority. Not to be confused with `archive/` at the root. | never (a later brief is a new dated file) | — |
...
| a capability (vendor, model, job, mail kind) | BUILD §6.3 closed list / §11 / §12 + `SPEC.md` §6.8 | `constants.ts` pin, the cost seam, the register (`MAIL_KINDS`, job registry) |
