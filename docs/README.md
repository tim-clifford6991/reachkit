...
without drifting from each other.
...
|---|---|---|---|---|
| 1 | `DECISIONS.md` | Every ruling, dated, one line each, newest last. Owner rulings are recorded verbatim in intent. | a ruling is made (owner) or a design/engineering decision closes (master) — batched into a docs PR | the drift audit cites its rows; `pending-decisions` is emptied by each batch |
...
| 4 | `ARCHITECTURE.md` | Where code lives: module table with public entry points, and the ten structural rules each with its check. | a module, directory or boundary is added or moved (in the same PR, docs exception) | eslint fences, `tests/app/toolchain.test.ts`, the drift audit |
...
| 8 | `docs/RUNBOOK.md` | Operating the product alone: jobs, kill switch, cost ledger, backups, incidents. | an operational procedure is added or rehearsed | — (rehearsals are dated inside it) |
...
2. **Amend in the PR that changes the fact.** A code PR that changes behaviour the spec describes carries the BUILD amendment (docs exception, master-reviewed); it does not queue it.
3. **Rulings are recorded the day they are made** by the master, and reach `DECISIONS.md` in the next docs batch (at most a few days apart).
