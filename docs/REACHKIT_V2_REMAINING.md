...
Status of the v2 build. M1–M5 are **built, tested (752 unit tests green), and on `main`**. Market analysis (M4 + M5) runs **unconditionally** — the old `REACHKIT_MARKET_ANALYSIS` gate has been removed.
...
- ☑ **M4** Gap analysis → distribution plan → paid report (4 sections)
- ☑ **M5** Execution layer — one-click handoff + draft engine + coach
- ☑ **Hardening** Backlinks off, persistent search cache, cache versioning
- ☑ **Bluesky removed**

## Env vars
**Local (`.env.local`)** — all keys present. To test v2 locally against real data set `REACHKIT_USE_FIXTURES=false`. (Market analysis is unconditional — no flag to enable.)
**Prod (Vercel)** — all keys present. `DATAFORSEO_BACKLINKS` optional.
