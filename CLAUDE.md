...
2. **Cost caps.** LLM/tool side (`ScanBudget`): 60 tool-calls; cents free 20 / full 250 / weekly 120; `BudgetExceededError` on breach. Both heavy cost points (deep scan + competitor-select) bounded to `MAX_SELECTED=5` rivals, de-duped only by per-domain caches. External side (DataForSEO + Tavily): every cost-bearing caller runs under a cost context (`costedStep` / `costedIntelStep` — guard: `app/api/costed-routes.test.ts`) so spend attributes to a scan row, AND a **per-scan SOFT cap** (free 25¢ / paid 150¢, `EXTERNAL_SCAN_CAP_CENTS_*`) bounds it: on breach `recordExternalCost` flips a flag (**never throws** — degrade, never invent), the pipeline's checkpoints (`externalCapBreached`) skip remaining external enrichment (market pass), and the scan is stamped `external_cap_hit_at` (visible on `/app/diagnostics`). Guards: `cost-context.test.ts` (breach semantics) + `documented-invariants.test.ts` (cap defaults). See `docs/architecture.md` §4.3.
...
| Design parity | `pnpm check:design` (`scripts/check-design-parity.mjs`) | Claude Design ↔ code: every `--c-*` in `app/globals.css` equals `.design-sync/tokens.css` (light+dark); `--c-band-*` equals `SCORE_BANDS`; **every ACTIVE `ds-src` must declare a resolving `/* @mirrors <live-path> */`** (primitives `-`; archived exempt); **mirror freshness** — warns (via `.design-sync/mirror-lock.json`) when a mirrored live file changed but its DS card didn't; and the **coverage ratchet** — the unmirrored-component count is pinned in `.design-sync/coverage-baseline.json` and HARD-FAILS if it grows (lower it when coverage improves). Re-bless with `pnpm bless:design` — it enumerates every re-pinned mirror, and `pnpm bless:design -- <Component…>` scopes the bless to named cards only. |
