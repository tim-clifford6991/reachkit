...
1. **Free↔paid headline stability.** As of PR #36 (`score_version 3`) the headline gauge is the **18-signal `registryScore`** (weighted avg over the *assessed* pillars) — the SAME number the pillar bars use, so the gauge == pillar average by construction. Free assesses only the on-site pillars (market null); deep adds the off-site market-position grade *alongside* (a separate number), so free→paid never switches scoring models or jumps the headline. (The old fixed-8-key `headlineScore` is retired-but-tested.) Guard: registry-score/headline tests.
