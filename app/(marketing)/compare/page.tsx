...
export default function ComparePage() {
  return (
    <main aria-label="Compare ReachKit" style={{ background: "#fff" }}>
      <section style={{ maxWidth: 1180, margin: "0 auto", padding: "70px 28px 24px", textAlign: "center" }}>
        <p style={{ fontFamily: JM, fontSize: 12, fontWeight: 600, letterSpacing: "0.12em", textTransform: "uppercase", color: "#6E56F7", margin: 0 }}>Compare</p>
        <h1 style={{ fontFamily: SG, fontWeight: 700, fontSize: "clamp(2rem, 4vw, 3.25rem)", letterSpacing: "-0.02em", lineHeight: 1.05, color: "#14131A", margin: "16px auto 0", maxWidth: 760 }}>
          How ReachKit stacks up
        </h1>
        <p style={{ fontSize: 17, lineHeight: 1.5, color: "#56535F", margin: "18px auto 0", maxWidth: 560 }}>
      ...
      <section style={{ maxWidth: 1180, margin: "0 auto", padding: "20px 28px 80px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 18 }}>
          {COMPARISONS.map((c) => (
            <Link key={c.slug} href={`/compare/${c.slug}`} style={{ display: "block", background: "#fff", border: "1px solid #ECEAF3", borderRadius: 16, padding: "26px 26px 22px", textDecoration: "none" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
                <span style={{ fontFamily: SG, fontWeight: 700, fontSize: 13, color: "#6E56F7" }}>ReachKit</span>
                <span style={{ fontSize: 13, color: "#9A97A5" }}>vs</span>
                <span style={{ fontFamily: SG, fontWeight: 700, fontSize: 13, color: "#14131A" }}>{c.name}</span>
              </div>
              <div style={{ fontFamily: SG, fontWeight: 700, fontSize: 20, letterSpacing: "-0.01em", color: "#14131A" }}>ReachKit vs {c.name}</div>
              <div style={{ fontSize: 12.5, fontWeight: 600, color: "#9A97A5", textTransform: "uppercase", letterSpacing: "0.05em", margin: "4px 0 12px" }}>{c.tagline}</div>
              <p style={{ fontSize: 14.5, lineHeight: 1.55, color: "#56535F", margin: 0 }}>{c.blurb}</p>
              <div style={{ marginTop: 16, fontFamily: "var(--font-sans)", fontWeight: 600, fontSize: 14, color: "#6E56F7" }}>See the comparison →</div>
            </Link>
          ))}
        </div>
      </section>
