 ...
 * ReachKit gets there. Written from the reader's value first; the full openness
 * (the real 18-signal registry with its pillar weights, imported from
 * lib/scan/signals — the same module the engine scores with — the
 * supply→demand→synthesis pipeline, and why the number is trustworthy) is kept
 * as a trust asset, not the thesis. Then a fixture-data glimpse of the paid
 * dashboard, ending on the free-scan CTA.
 *
...
const STEPS = [
  { n: "01", title: "Scan your live page", body: "Paste your App Store URL or website. ReachKit reads the real page — the same signals search engines and buyers weigh — and scores all 18 in about 90 seconds. No account needed for your first scan.", tag: "~90 seconds" },
    ...
    <main aria-label="How ReachKit works" style={{ background: "var(--c-surface)" }}>
      ...
      <section id="pipeline" style={{ maxWidth: 1180, margin: "0 auto", padding: "64px 28px 20px", scrollMarginTop: 88 }}>
        <SectionHead
          ...
          sub="Every scan runs three passes: what's already out there (you vs. your competitors), what your buyers are actually searching for, and the specific plays that fall out of crossing the two. The score is deterministic; every recommendation cites the evidence behind it."
        />
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 18 }}>
          {PIPELINE.map((p) => (
      ...
      <section style={{ maxWidth: 1180, margin: "0 auto", padding: "40px 28px 90px" }}>
        <div style={{ background: "linear-gradient(135deg, var(--c-dark), var(--c-dark2))", borderRadius: 22, padding: "48px 32px", textAlign: "center" }}>
          ...
          <p style={{ fontSize: 15.5, lineHeight: 1.6, color: "var(--c-on-dark-muted)", margin: "0 auto 24px", maxWidth: 560 }}>
            The free scan gives you your discoverability score and your top fixes right now. Paid plans run the whole engine every week — the ranked queue, the drafts, the verification, your score history — so you&rsquo;re never guessing what to do next.
          </p>
